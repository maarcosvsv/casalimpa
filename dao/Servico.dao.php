...
class ServicoDAO {
 ...
 and serv.ativo = 1";
    ...
    function getOrdemServicoPorCodigo($ordemServico) {
        $connectionFactory = new connectionFactory();
        $connection = $connectionFactory->getConnection();
        $sqlServico = "select distinct os.*, p.nome as nomeProfissional, c.nome as nomeCliente, 
...
and cidade.PK_Cidade = bairro.TB_Cidade_PK_Cidade
 ...
 and serv.cod_servico = " . $codServico;
    ...
    function contratarServico($idUsuario, $codServico, $dataInicio, $dataFim, $turnoPreferencia, $idLogradouro, $horario, $numeroOS) {
        $connectionFactory = new connectionFactory();
        $connection = $connectionFactory->getConnection();
